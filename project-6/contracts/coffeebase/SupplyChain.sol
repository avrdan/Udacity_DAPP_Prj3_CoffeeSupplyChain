...
contract SupplyChain {
  ...
  {
    //bytes memory bytesData = abi.encodePacked(bytes32(_upc), bytes("_"));
    //string memory productID = string(abi.encodePacked(bytesData, bytes32(sku)));

    // Add the new item as part of Harvest
    items[_upc] = Item(sku, _upc, msg.sender, _originFarmerID, _originFarmName,
      _originFarmInformation, _originFarmLatitude, _originFarmLongitude, _upc + sku, 
      _productNotes, 0 /* productPrice*/, State.Harvested, address(0), address(0), address(0));
    // Increment sku
    sku = sku + 1;
  ...
  // Define a function 'sellItem' that allows a farmer to mark an item 'ForSale' (i.e: addItem)
